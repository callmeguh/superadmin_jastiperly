...
<div class="d-flex flex-wrap gap-2 mb-3 align-items-center">
    ...
    <!-- Filter Jenis -->
    <a href="{{ route('superadmin.transactions', array_merge(request()->except('type'), ['type' => 'beli'])) }}" 
       class="btn btn-sm shadow-sm {{ request('type')=='beli' ? 'btn-primary text-white' : 'btn-light' }} border rounded p-2">
        Titip Beli <span class="badge bg-primary">{{ $countBeli ?? 0 }}</span>
    </a>
    <a href="{{ route('superadmin.transactions', array_merge(request()->except('type'), ['type' => 'kirim'])) }}" 
       class="btn btn-sm shadow-sm {{ request('type')=='kirim' ? 'btn-success text-white' : 'btn-light' }} border rounded p-2">
        Titip Kirim <span class="badge bg-success">{{ $countKirim ?? 0 }}</span>
    </a>

    <!-- Filter Lingkup -->
    <form method="GET" action="{{ route('superadmin.transactions') }}">
        <select name="scope" class="form-select form-select-sm shadow-sm border rounded p-2" onchange="this.form.submit()">
    ...
    <!-- Filter Status -->
    <form method="GET" action="{{ route('superadmin.transactions') }}">
        <select name="status" class="form-select form-select-sm shadow-sm border rounded p-2" onchange="this.form.submit()">
    ...
    <a href="{{ route('superadmin.transactions.export', request()->all()) }}" 
       class="btn btn-sm btn-success fw-semibold shadow-sm p-2 d-flex align-items-center">
    ...
    <a href="{{ route('superadmin.transactions.show', $transaction->id) }}" 
       class="btn btn-sm btn-info me-1 p-1 px-2" style="font-size:0.75rem;" title="Detail">
        Detail
    </a>
    <a href="{{ route('superadmin.transactions.edit', $transaction->id) }}" 
       class="btn btn-sm btn-warning me-1 p-1 px-2" style="font-size:0.75rem;" title="Edit">
        Edit
    </a>
    <form action="{{ route('superadmin.transactions.destroy', $transaction->id) }}" method="POST" 
        ...
        @csrf
        @method('DELETE')
        <button type="submit" class="btn btn-sm btn-danger p-1 px-2" style="font-size:0.75rem;" title="Hapus">
            Hapus
        </button>
    </form>
</td>
