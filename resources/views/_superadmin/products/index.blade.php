    ...
    <div class="card-body">
        ...
        <div class="d-flex justify-content-between align-items-center mb-4 flex-wrap gap-2">
            <div class="d-flex gap-2 flex-wrap">
                <a href="{{ route('superadmin.products', ['role' => 'traveler']) }}"
                   class="btn btn-sm {{ request('role')=='traveler' ? 'btn-primary text-white' : 'btn-outline-primary' }} px-3">
                    Traveler 
                    <span class="badge bg-light text-dark ms-1">
                        {{ \App\Models\Product::whereHas('traveler', fn($q) => $q->where('role','traveler'))->count() }}
                    </span>
                </a>
                <a href="{{ route('superadmin.products', ['role' => 'customer']) }}"
                   class="btn btn-sm {{ request('role')=='customer' ? 'btn-success text-white' : 'btn-outline-success' }} px-3">
                    Customer 
                    <span class="badge bg-light text-dark ms-1">
                        {{ \App\Models\Product::whereHas('traveler', fn($q) => $q->where('role','customer'))->count() }}
                    </span>
                </a>
            </div>

            <form method="GET" class="d-flex align-items-center gap-2 flex-wrap">
                <select name="status" class="form-select form-select-sm w-auto shadow-sm border rounded-pill" onchange="this.form.submit()">
                    <option value="">Semua Status</option>
                    <option value="validasi" {{ request('status')=='validasi'?'selected':'' }}>Validasi</option>
                    <option value="disetujui" {{ request('status')=='disetujui'?'selected':'' }}>Disetujui</option>
                    <option value="ditolak" {{ request('status')=='ditolak'?'selected':'' }}>Ditolak</option>
                </select>
                <a href="{{ route('superadmin.products.export') }}" class="btn btn-sm btn-warning rounded-pill d-flex align-items-center">
        ...
        <div class="list-group">
            @forelse($products as $product)
                <div class="list-group-item border rounded-3 shadow-sm mb-3 p-3 d-flex align-items-center gap-3"
                     style="transition: transform 0.15s ease-in-out, box-shadow 0.2s ease-in-out;"
                     onmouseover="this.style.transform='scale(1.01)'; this.style.boxShadow='0 4px 12px rgba(0,0,0,0.12)';"
                     onmouseout="this.style.transform='scale(1)'; this.style.boxShadow='0 1px 6px rgba(0,0,0,0.08)';">

                    <!-- Foto Produk -->
                    <img src="{{ $product->foto ? asset('storage/'.$product->foto) : 'https://via.placeholder.com/120x100' }}"
                         class="shadow-sm border"
                         style="width: 120px; height: 100px; object-fit: cover;" alt="Produk">

                    <!-- Detail Produk -->
                    <div class="flex-grow-1">
                        <h6 class="fw-semibold text-dark mb-1">{{ $product->nama_barang }}</h6>
                        <p class="mb-1 text-muted small">{{ $product->deskripsi_barang }}</p>
                        <p class="mb-1"><strong>Harga:</strong> Rp {{ number_format($product->harga_barang, 0, ',', '.') }}</p>
                        <p class="mb-1"><strong>Traveler:</strong> {{ $product->traveler->name ?? '-' }}</p>
                        <span class="badge 
                            @if($product->status=='validasi') bg-warning text-dark
                            @elseif($product->status=='disetujui') bg-success
                        ...
                        @if($product->status == 'validasi')
                            <form action="{{ route('superadmin.products.validate', $product->id) }}" method="POST">
                                ...
                                <button class="btn btn-sm btn-outline-success rounded-pill d-flex align-items-center">
                                    <iconify-icon icon="mdi:check-circle-outline" class="me-1"></iconify-icon> Validasi
                                </button>
                            </form>
                        @endif
                        <a href="#" class="btn btn-sm btn-outline-info rounded-pill d-flex align-items-center">
